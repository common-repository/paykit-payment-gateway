...
class Paykit_Retrieve_Payment_Response extends Paykit_Response
{
  ...
  {
    $refunds = array_map(fn($value) => Paykit_Refund::from_json($value), $data['refunds'] ?? array());

    return new Paykit_Retrieve_Payment_Response(
      $data['result'],
      $data['gateway_code'],
      $data['error'] ?? null,
